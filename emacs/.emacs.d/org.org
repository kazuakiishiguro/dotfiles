...
#+begin_src emacs-lisp
 ...
 (setq org-capture-templates
	 ...
	 ("n" "Note" plain (file (lambda ()
	   (expand-file-name
	    (concat (replace-regexp-in-string " " "_" (read-string "Note title: ")) ".org")
	    "~/org")))
	  "#+TITLE: %^{Title}\n#+DATE: %U\n#+FILETAGS:\n\n[[file:Daily/%(format-time-string \"%%Y-%%m-%%d\").org][%(format-time-string \"%%Y-%%m-%%d\")]]\n\n%?")

	 ("r" "Reference" plain (file (lambda ()
	   (expand-file-name
	    (concat (replace-regexp-in-string " " "_" (read-string "Reference title: ")) ".org")
	    "~/org/References")))
	  "#+TITLE: %^{Title}\n#+DATE: %U\n#+FILETAGS: :%(my/reference-categories):\n\n%?")

	 ("K" "Category" plain (file (lambda ()
	   (expand-file-name
	    (concat (replace-regexp-in-string " " "_" (read-string "Category name: ")) ".org")
	    "~/org")))
	  "#+TITLE: %^{Title}\n#+DATE: %U\n#+TYPE: category\n\n%?")

	 ("c" "Clipping" plain (file (lambda ()
	   (expand-file-name
	    (concat (replace-regexp-in-string " " "_" (read-string "Clipping title: ")) ".org")
	    "~/org/Clippings")))
