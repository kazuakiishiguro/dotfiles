 ...
 (defun my/org-capture-add-daily-backlink ()
   ...
   (when (and (equal (plist-get org-capture-plist :key) "n")
     ...
     (let* ((daily-dir (expand-file-name "Daily" org-directory))
            (daily-file (expand-file-name (format-time-string "%Y-%m-%d.org") daily-dir))
            (note-file (expand-file-name
                        (concat (my/org-title-to-filename my/capture-last-title) ".org")
                        org-directory))
            (rel-path (file-relative-name note-file (file-name-directory daily-file)))
            (link (format "[[file:%s][%s]]" rel-path my/capture-last-title)))
       ...
       (with-current-buffer (find-file-noselect daily-file)
         ...
         (goto-char (point-max))
         (unless (search-backward link nil t)
           (goto-char (point-max))
           (unless (bolp) (insert "\n"))
           (insert (format "- %s\n" link)))
         (save-buffer)))))
...
#+begin_src emacs-lisp
 (defun my/set-filetags ()
   ...
   (interactive)
   (let ((value (concat ":" (mapconcat #'identity (my/select-categories "Tags") ":") ":")))
         (save-excursion
           (goto-char (point-min))
           (if (re-search-forward "^#\\+FILETAGS:.*$" nil t)
               (replace-match (concat "#+FILETAGS: " value))
             ;; Insert after the last #+KEYWORD line in the header
             (goto-char (point-min))
